...
export class AllocationService {
  ...
  async findAll(filters: {
    ...
    if (filters.userId) {
      qb.andWhere(
        '(allocation.ALOCADO_POR = :userId OR allocation.FINALIZADO_POR = :userId)',
        {
  ...
  async alocar(
    ...
    await this.addLog(
      order.ID,
      name,
      `Alocou ${this.stagesService.getLabel(stage)}`,
    );
  ...
  async finalizar(
    ...
    if (order.STAGE !== stage) {
      throw new BadRequestException(
        'Só é possível finalizar a etapa atual do pedido',
      );
    }
    ...
    await this.addLog(
      order.ID,
      user.NOME,
      `Finalizou ${this.stagesService.getLabel(stage)}`,
    );
    ...
    if (next) {
      ...
      await this.addLog(
        order.ID,
        user.NOME,
        `Pedido movido para ${this.stagesService.getLabel(next)}`,
      );
    }
  ...
  async move(
    ...
    await this.addLog(
      order.ID,
      user.NOME,
      `Moveu o pedido para ${this.stagesService.getLabel(stage)}`,
    );
  ...
  private async upsertAllocation(
    ...
    let allocation = await this.allocationsRepo.findOne({
      where: { ORDER_ID: orderId, STAGE: stage },
    });
    if (!allocation) {
      allocation = this.allocationsRepo.create({
        ORDER_ID: orderId,
        STAGE: stage,
      });
